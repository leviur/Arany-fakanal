# Nyitvatartás logika — DB-ből olvassa ki, mikor vagyunk nyitva.
# Asztalfoglaláskor ellenőrzi: az adott nap+idő rendben van-e.
# A frontend (opening-hours.js) ugyanezt mutatja a dropdownban; itt a backend is megnézi.
...
# hétfő=0 … vasárnap=6 → "monday" stb. (WeeklyOpeningHour.day mező)
...
def ensure_default_weekly_hours():
    # Ha még nincs nyitvatartás az DB-ben, feltölti az alapértelmezettel (constants.py).
    # Ha már van adat, nem nyúl hozzá — views.py hívja GET/PUT előtt.
...
def get_day_hours_info(target_date):
    # Adott nap: zárva vagyunk-e, meddig nyitva. Dict, vagy None ha nincs adat.
    # Először kivétel nap (ünnep), aztán a szokásos heti sablon.
...
def get_time_slots_for_date(target_date):
    # 30 percenkénti időpontok: pl. 11:00, 11:30, 12:00 … zárásig.
    # Zárva nap → üres lista. (opening-hours.js-ben ugyanígy számoljuk a fronton.)
...
def validate_reservation_slot(reservation_date, reservation_time):
    # Foglalás mentés előtt: nyitva vagyunk-e, és az idő benne van-e a slot listában.
    # reservations/serializers.py + guest_portal hívja. A dropdown önmagában nem elég védelem.
...
def ensure_sla_settings():
    # Dashboard „mikor legyen piros a sor?” küszöbök — külön tábla (SlaSettings), nem nyitvatartás.
    # Mindig csak 1 sor lehet (pk=1). Ha friss telepítésnél még üres az DB, létrehozza az alapértékekkel.
    # GET/PUT /api/sla-rules/ előtt hívjuk, hogy ne dobjon hibát „nincs ilyen sor”.
